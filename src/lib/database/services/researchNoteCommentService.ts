...
import type { ResearchNoteComment } from '../../supabase'
...
export const createResearchNoteComment = async (
  ...
  if (!isSupabaseConfigured || !supabase) {
    // Local storage fallback
    try {
      const comments = JSON.parse(localStorage.getItem('kyp_research_note_comments') || '[]')
...
export const updateResearchNoteComment = async (
  ...
  if (!isSupabaseConfigured || !supabase) {
    ...
    try {
      ...
      const updatedComment = updatedComments.find((c: ResearchNoteComment) => c.id === commentId)
